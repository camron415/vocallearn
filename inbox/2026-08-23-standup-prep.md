# Standup prep
**Date:** 2026-08-23T12:01:00.782Z · **Project:** vocallearn
**Analyst job:** aj-1787486442951-w5zcdf

# Standup prep — Halo / VocalLearn
**Date:** 2026-08-23T12:01:00.782Z · **Manager:** Sloane

standup: active

## Since yesterday
- Shipped the Cove web site so early-access invites can now go out.
...
## Roadmap position
- We're on the Halo / Cove track; the live shipping surface is the family Ask website.
- Next real outcome is a protected commit of the 35-file working tree so the live site stays safe in git.

## Proposed today (P / S)
1. Approve committing the 35-file working tree so the live Halo site is protected in git — P0
2. Read the pricing document and confirm the $1-per-user-per-week ceiling is still met under current Ask routing — P1
3. Decide whether the next native build should target the Ask-to-Practice loop or continue Halo motion polish — P2

## Decisions for Camron
Do you want to commit the 35-file working tree today, yes or no?
...
### Analyst findings
## Headline
Lab priorities and auto-queue logic for VocalLearn are not yet visible in the provided context, so the $1/user/week cost ceiling for the live Halo Ask surface cannot be confirmed as preserved without inspecting the canonical pricing document.

## Key findings
- **Canonical docs**: `inbox/2026-08-21-halo-pricing-and-scaling.md` explicitly directs all agents to read `docs/HALO_PRICING_AND_SCALING.md` for API cost per user with hybrid routing, recommended tiers, and revenue scenarios at 100 users.
- **Git activity**: HEAD at b7b497b (“Ship Cove web so early-access invites can go out”) after c0cc60e and 2d7acf9; 35 files changed, 857 insertions, 593 deletions on branch `halo-ui-streamline`.
- **Priority implementation files**: `web/src/lib/ask-turn.ts`, `web/src/lib/suggest-chips.ts`, `web/src/app/api/chat/route.ts`, `web/src/components/InviteSetup.tsx`, and `web/src/lib/water-edge.ts` contain the new Ask + Halo code referenced in the outcome statements.
- **Phase status**: `docs/PHASE_1_75.md` is referenced but not loaded; `inbox/2026-08-10-ask-practice-roadmap.md` states Phase 0–3 implemented and awaiting device smoke-test.
- **Recent commits** confirm the web site is the current shipping surface; native Ask→Practice work remains in the working tree.

## Risks or gaps
- **Cost ceiling not visible**: No excerpts from `docs/HALO_PRICING_AND_SCALING.md` or `docs/HALO_PRICING_AND_SCALING.md` are present, so the $1/user/week target cannot be validated against current routing or model choices.
...
- **Next file to inspect if uncertain**: `docs/HALO_PRICING_AND_SCALING.md` to confirm whether the $1/user/week ceiling is preserved under the current hybrid routing.

## Suggested next steps
1. **Camron voice decision (P0)**: Approve commit of the 35-file working tree so the live Halo site is protected in git.
2. **Planning session item (P1)**: Read `docs/HALO_PRICING_AND_SCALING.md` and confirm the $1/user/week ceiling is still met under the current Ask routing and model choices.
3. **Planning session item (P2)**: Decide whether the next native build should target the Ask→Practice loop (Phase 3 smoke-test) or continue Halo H3 motion polish.
4. **Planning session item (P3)**: Schedule a 30-minute device test of the Ask→Practice loop once the fresh IPA is installed.
5. **Planning session item (P4)**: Inspect `supabase/migrations/` to verify Phase 0 tables are applied before any further Ask feature work.
